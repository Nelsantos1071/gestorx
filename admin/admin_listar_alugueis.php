...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        } elseif (isset($_POST['editar_aluguel'])) {
            $aluguel_id = (int)$_POST['aluguel_id'];
            $data_fim = trim($_POST['data_fim']);

            if (empty($data_fim)) {
                $error = "Data de Fim é obrigatória.";
            } elseif (strtotime($data_fim) < strtotime(date('Y-m-d'))) {
                $error = "Data de Fim não pode ser anterior à data atual.";
            } else {
                $stmt = $pdo->prepare("UPDATE alugueis SET data_fim = ? WHERE id = ?");
                $stmt->execute([$data_fim, $aluguel_id]);
                $success = "Data de Fim do aluguel atualizada com sucesso!";
                error_log("Data de Fim atualizada: aluguel_id=$aluguel_id, data_fim=$data_fim");
                header("Location: admin_listar_alugueis.php?success=" . urlencode($success));
                exit;
            }
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .action-button[href="admin_alugueis.php"] { display: block; margin: 0 auto 15px; width: fit-content; }
...
<body>
    ...
    <main role="main" tabindex="0">
        ...
        <section class="alugueis-section" aria-label="Lista de aluguéis">
            ...
            <a href="admin_alugueis.php" class="action-button">Adicionar Aluguel</a>
                    ...
                    <tbody>
                        <?php foreach ($alugueis as $aluguel): ?>
                            ...
                            <tr class="<?php echo $aluguel['status'] === 'inativo' ? 'inativo' : ''; ?>">
                                ...
                                <td data-label="Ações">
                                    <button class="action-button" onclick="openModal(<?php echo $aluguel['id']; ?>, <?php echo $aluguel['cliente_id']; ?>, '<?php echo htmlspecialchars($aluguel['cliente_nome'] ?? '', ENT_QUOTES); ?>', '<?php echo htmlspecialchars($aluguel['status'], ENT_QUOTES); ?>', '<?php echo $aluguel['data_fim'] ? htmlspecialchars($aluguel['data_fim'], ENT_QUOTES) : ''; ?>')">Ações</button>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
            ...
            <div class="modal-content">
                ...
                <div id="edit-cliente-section" style="display: none;">
                    ...
                    <form method="POST" action="admin_listar_alugueis.php">
                        ...
                        <button type="submit" name="editar_cliente" class="edit-button">Salvar Alterações do Cliente</button>
                    </form>
                </div>
                <div id="edit-aluguel-section" style="display: block;">
                    <h4>Editar Aluguel</h4>
                    <form method="POST" action="admin_listar_alugueis.php">
                        <input type="hidden" name="aluguel_id" id="modal-aluguel-id-edit">
                        <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token']); ?>">
                        <label for="modal-data-fim">Data de Fim:</label>
                        <input type="date" name="data_fim" id="modal-data-fim" required>
                        <button type="submit" name="editar_aluguel" class="edit-button">Salvar Data de Fim</button>
                    </form>
                </div>
                <div id="status-section">
                    <h4>Gerenciar Status</h4>
                    <form method="POST" action="admin_alugueis.php" style="display: inline;">
    ...
    <script>
        function openModal(aluguelId, clienteId, clienteNome, status, dataFim) {
            ...
            const aluguelSection = document.getElementById('edit-aluguel-section');
            const aluguelIdInput = document.getElementById('modal-aluguel-id');
            const aluguelIdInputEdit = document.getElementById('modal-aluguel-id-edit');
            const clienteIdInput = document.getElementById('modal-cliente-id');
            const nomeInput = document.getElementById('modal-nome');
            const dataFimInput = document.getElementById('modal-data-fim');
            ...
            aluguelIdInputEdit.value = aluguelId;
