...
try {
    ...
    $stmt = $pdo->query("SELECT id, titulo FROM produtos_servicos WHERE tipo = 'servico' ORDER BY titulo");
    $servicos = $stmt->fetchAll(PDO::FETCH_ASSOC);
    error_log("Serviços encontrados: " . count($servicos));
    $servicos_disponiveis = !empty($servicos);

    if (!$servicos_disponiveis) {
        $error = "Nenhum serviço disponível para aluguel.";
    }
} catch (PDOException $e) {
    $error = "Erro ao buscar dados: " . htmlspecialchars($e->getMessage()) . " (Code: " . $e->getCode() . ")";
    error_log($error);
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['adicionar_aluguel'])) {
    try {
        ...
        $url_download = filter_var(trim($_POST['url_download']), FILTER_SANITIZE_URL) ?: null;
        $site = filter_var(trim($_POST['site']), FILTER_SANITIZE_URL) ?: null;
        $usuario = filter_var(trim($_POST['usuario']), FILTER_SANITIZE_STRING, FILTER_FLAG_NO_ENCODE_QUOTES) ?: null;
        $senha = trim($_POST['senha']) ? password_hash(trim($_POST['senha']), PASSWORD_DEFAULT) : null;
        $chave_pix = filter_var(trim($_POST['chave_pix']), FILTER_SANITIZE_STRING, FILTER_FLAG_NO_ENCODE_QUOTES) ?: null;
        ...
        } elseif ($usuario && strlen($usuario) > 50) {
            $error = "O campo usuário deve ter no máximo 50 caracteres.";
        } elseif ($chave_pix && strlen($chave_pix) > 100) {
            $error = "O campo chave Pix deve ter no máximo 100 caracteres.";
        } else {
            $stmt = $pdo->prepare("
                INSERT INTO alugueis (cliente_id, servico_id, quantidade, data_aluguel, status, url_download, site, usuario, senha, chave_pix)
                VALUES (?, ?, ?, NOW(), 'ativo', ?, ?, ?, ?, ?)
            ");
            $stmt->execute([$cliente_id, $servico_id, $quantidade, $url_download, $site, $usuario, $senha, $chave_pix]);
            $success = "Aluguel registrado com sucesso!";
            error_log("Aluguel inserido: cliente_id=$cliente_id, servico_id=$servico_id, quantidade=$quantidade, url_download=" . ($url_download ?: 'null') . ", site=" . ($site ?: 'null') . ", usuario=" . ($usuario ?: 'null') . ", senha=" . ($senha ? '[hashed]' : 'null') . ", chave_pix=" . ($chave_pix ?: 'null'));
            header("Location: admin_listar_alugueis.php?success=" . urlencode($success));
            exit;
        }
    } catch (PDOException $e) {
        $error = "Erro ao registrar aluguel: " . htmlspecialchars($e->getMessage()) . " (Code: " . $e->getCode() . ")";
        error_log($error);
    }
}
?>
...
<body>
        ...
        <?php else: ?>
            <section class="form-section" aria-label="Formulário de Aluguel">
                ...
                <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="POST">
                    ...
                    <input type="text" name="usuario" id="usuario" placeholder="Digite o usuário" maxlength="50">
                    ...
                    <input type="text" name="chave_pix" id="chave_pix" placeholder="Ex.: 12345678900 ou email@example.com" maxlength="100">
    ...
    <script>
        document.querySelector('form').addEventListener('submit', function(e) {
            const urlDownload = document.getElementById('url_download').value;
            const site = document.getElementById('site').value;
            const urlPattern = /^https?:\/\/[^\s$.?#].[^\s]*$/;

            if (urlDownload && !urlPattern.test(urlDownload)) {
                e.preventDefault();
                alert('URL de download inválido.');
            }
            if (site && !urlPattern.test(site)) {
                e.preventDefault();
                alert('URL do site inválido.');
            }
        });
    </script>
</body>
